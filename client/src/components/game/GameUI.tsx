...
export function GameUI() {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex items-center justify-between bg-black/40 backdrop-blur-sm pointer-events-auto z-10 border-b border-primary/20">
        <div className="flex items-center space-x-4">
          {/* Level Badge */}
          <div className="bg-card p-2 rounded-full h-12 w-12 flex items-center justify-center border-2 border-primary">
            <div className="text-lg font-bold text-primary">{currentLevel}</div>
          </div>
          
          {/* XP Bar */}
          <div className="bg-card/80 p-2 rounded-md border border-primary/30 w-48">
            <div className="text-xs text-primary/90 uppercase tracking-wider mb-1 font-bold">Experience</div>
            <ProgressIndicator 
              value={experience.current} 
              max={experience.required} 
              label={`${experience.current}/${experience.required} XP`} 
              className="h-3"
            />
          </div>
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="bg-card hover:bg-card/80 px-3 py-2 rounded-md text-sm font-medium flex items-center gap-2 border border-primary/30 neon-glow"
          >
          ...
          <Link 
            to="/profile" 
            className="bg-card hover:bg-card/80 px-3 py-2 rounded-md text-sm font-medium flex items-center gap-2 border border-accent/30"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {showEditor && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/80 pointer-events-auto z-50">
          <div className="bg-card w-full max-w-5xl mx-4 h-[85vh] rounded-lg flex flex-col overflow-hidden border border-primary/30 shadow-2xl">
            {/* Challenge header */}
            <div className="p-4 bg-primary text-primary-foreground border-b border-primary/20">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="px-2 py-1 bg-primary-foreground/20 text-xs rounded-md">
                  ...
                  <button 
                    ...
                    className="hover:bg-primary-foreground/20 p-2 rounded-full"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <line x1="18" y1="6" x2="6" y2="18"></line>
                      <line x1="6" y1="6" x2="18" y2="18"></line>
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-1/3 p-4 bg-card-foreground/5 overflow-y-auto border-r border-primary/10">
                <h3 className="font-bold text-lg mb-2 text-secondary">Description</h3>
                <p className="mb-4 text-foreground/90">{currentChallenge?.description}</p>
                
                {currentChallenge?.example && (
                  <div className="mb-4">
                    <h3 className="font-bold text-lg mb-2 text-secondary">Example</h3>
                    <pre className="bg-black/30 p-3 rounded text-sm overflow-x-auto font-code">
                      {currentChallenge.example}
                    </pre>
                  </div>
                )}
                
                <div className="bg-card p-4 rounded-md border border-primary/20">
                  <h3 className="font-bold text-sm mb-2 text-primary">Task</h3>
              ...
              <div className="w-full md:w-2/3 flex flex-col">
                ...
                <div className="h-36 bg-black/60 border-t border-primary/20 overflow-hidden">
                  <div className="p-3 text-xs uppercase tracking-wider font-bold flex justify-between items-center text-foreground/70 bg-black/40">
                    <span>Output</span>
                    <span className="text-primary">{lastOutputTime > 0 ? new Date(lastOutputTime).toLocaleTimeString() : ''}</span>
                  </div>
      ...
      {showToast && (
        <div 
          className={`fixed top-4 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-md shadow-lg z-50 pointer-events-auto animate-fadeInDown text-white font-medium flex items-center gap-2 ${
